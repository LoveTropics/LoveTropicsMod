...
public class MinigameGui {
	...
	public static void renderGameOverlay(RenderGameOverlayEvent event) {
		ClientMinigameState.get().ifPresent(state -> {
			...
			if (state.getStatus() == MinigameStatus.ACTIVE && state.isJoined()) return;
			...
			if (event.getType() == ElementType.TEXT) {
				...
				String str = "Minigame: " + state.getDisplayName();
				fnt.drawStringWithShadow(str, padding, y, -1);
				y += lineHeight;
				str = "State: " + state.getStatus().name();
				fnt.drawStringWithShadow(str, padding, y, -1);
			}
		});
	}
