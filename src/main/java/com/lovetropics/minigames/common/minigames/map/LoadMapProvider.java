...
import com.lovetropics.minigames.common.dimension.RuntimeDimensionConfig;
	...
	public CompletableFuture<MinigameResult<MinigameMap>> open(MinecraftServer server) {
		...
		return RuntimeDimensions.get(server).openTemporary(new RuntimeDimensionConfig(dimension, 0, worldInfo))
				...
				.thenApplyAsync(pair -> {
					...
					MinigameMap map = new MinigameMap(name, dimensionHandle.asKey(), metadata.regions)
							.onClose(minigame -> dimensionHandle.delete());

					return MinigameResult.ok(map);
				}, server);
	}
}
