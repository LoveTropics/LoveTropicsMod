...
public final class GameEventDispatcher {
	...
	public void onChunkLoad(ChunkEvent.Load event) {
		if (event.getWorld() instanceof IServerWorld) {
			...
			IGameInstance game = gameLookup.getGameAt(world, event.getChunk().getPos().asBlockPos());
			if (game != null) {
	...
	public void onPlayerHurt(LivingHurtEvent event) {
		...
		if (entity instanceof ServerPlayerEntity) {
			IGameInstance game = gameLookup.getGameFor(entity);
			if (game != null) {
				try {
	...
	public void onEntityPlaceBlock(BlockEvent.EntityPlaceEvent event) {
		IGameInstance game = gameLookup.getGameFor(event.getEntity());
	...
	public void onExplosion(ExplosionEvent.Detonate event) {
		IGameInstance game = gameLookup.getGameAt(event.getWorld(), new BlockPos(event.getExplosion().getPosition()));
		if (game != null) {
			try {
